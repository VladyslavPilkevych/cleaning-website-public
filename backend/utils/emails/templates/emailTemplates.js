  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
      ...
      <p style="font-size: 16px;">${
        isPaymentReceived
          ? paymentTranslations[language].paymentReceived(amountFormatted)
          : paymentTranslations[language].paymentInCash(amountFormatted)
      }</p>
      <p style="font-size: 16px;">${
        paymentTranslations[language].paymentConfirmed
      }</p>
      <hr style="margin: 20px 0;">
      <p style="font-size: 15px; color: #555;">${
        paymentTranslations[language].paymentContact
      }</p>
      <p style="margin-top: 30px; font-size: 14px; color: #999;">${
        paymentTranslations[language].paymentReceiptNote
      }</p>
      <p style="font-size: 14px; color: #999;">${
        paymentTranslations[language].paymentFooter
      }</p>
    </div>
  `;
}
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
      ...
      <ul style="font-size: 15px; color: #333;">
        ...
        <li><strong>Прибирання поетапно:</strong> ${formatBool(
          formData.property.steps
        )}</li>
      </ul>

      <h3 style="color: #4CAF50;">Миття вікон</h3>
      <ul style="font-size: 15px; color: #333;">
        <li><strong>Потрібно миття:</strong> ${formatBool(
      ...
      <ul style="font-size: 15px; color: #333;">
        <li><strong>Потрібна:</strong> ${formatBool(
